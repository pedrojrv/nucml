...
from nucml.exfor.data_utilities import expanding_dataset_energy
from nucml.exfor.querying_utils import load_samples, load_newdata
from nucml.ace.querying_utils import get_energies
...
def ml_results_plotly(results_dict, order_dict={}, save=False, render_browser=False, show=False):
    ...
    if "exfor_ml_new" in results_dict.keys():
        exfor_ml_new_trace = go.Scattergl(
            x=results_dict["exfor_ml_new"]["df"].Energy, y=results_dict["exfor_ml_new"]["df"].Data, mode='markers',
            name='EXFOR New')
...
def ml_results_sns(results_dict, order_dict={}, save=False, render_browser=False, show=False):
    """Plot the machine learning predictions from the dictionary generated by the.
...
def make_chlorine_paper_figure(df, dt_model, dt_scaler, knn_model, knn_scaler, to_scale):
    ...
    chlorine_35_np_knn = load_samples(df, 17, 35, "MT_103", scaler=knn_scaler, **kwargs)
    chlorine_35_np_dt = load_samples(df, 17, 35, "MT_103", scaler=dt_scaler, **kwargs)

    new_cl_data_kwargs = {
        "Z": 17, "A": 35, "MT": "MT_103", "log": True, "to_scale": to_scale, "one_hot": True}
    new_cl_data_knn = load_newdata(
        "../EXFOR/New_Data/Chlorine_Data/new_cl_np.csv", df, scaler=knn_scaler, **new_cl_data_kwargs)
    new_cl_data_dt = load_newdata(
    ...
    for np_data, model, plot_axis, label, new_data in zip(
        ...
        chlorine_data_ext = expanding_dataset_energy(np_data, (0, 0), False, 0, e_array=ace_cl)
