"""Utility functions for model building and evaluation."""
import pandas as pd
import os
import shutil
...
def regression_error_metrics(v1, v2):
    """Calculate the MAE, MSE, EVS, MAEM, and R2 between two vectors.
...
def create_error_df(identifier, error_metrics_dict):
    """Create a simple dataframe from the performance metrics dictionary yielded by the regression_error_metrics().
...
def create_train_test_error_df(identifier, train_error_metrics, test_error_metrics, val_error_metrics=None):
    """Create a pandas DataFrame containing the error metrics provided by both the train and test dictionaries.

    generated by the regression_error_metrics() function. A validation error metrics dictionary can also be provided.
...
def make_predictions(data, model, model_type):
    """Make prediction using a trained model. Currently handles tensorflow, xgboost, and scikit-learn models.
...
def get_best_models_df(results_df, keep_first=False):
    """Return a three row minimum dataframe with the best models.

    It is based on training, validation, and testing performance metrics. The results_df argument is based on the file
    generated by the python training scripts
...
def load_model_and_scaler(model_scaler_info, df=True, model_only=False):
    """Load both the model and scaler given a dataframe with path's specified.
...
def cleanup_model_dir(results_df, model_dir, keep_best=True, keep_first=False):
    """Delete unwanted models and scalers.

    Keeps best models based on training, validation, and testing perfromance if wanted.
    ...
    Args:
        ...
        keep_first (bool, optional): If True, it will keep the first appearance in case of a duplicate rows. Defaults
            to False.
...
def remove_unused_models(model_results_path, acedate_directory):
    """Find best models in terms of train, validation and testing sets and deletes all others.

    It also keeps the best models in terms of multiplication factor.

    WARNING: Once deleted, other models will not be accessible.

    Args:
        model_results_path (str): Filepath to model training results CSV file generated using the model training
            scripts.
    ...
    to_keep = []
    to_keep.extend(
        list(model_results_df.iloc[model_results_df.sort_values(by="Deviation_Ana").head().index].Model.values))
    ...
    to_remove = model_results_df[model_results_df.filtering is True]
...
def filter_by_parameters(results_df, param_dict):
    """Filter dataframe given a parameter dictionary."""
    for i in param_dict:
...
def get_parameters_from_line(results_df, model="knn"):
    """Get the parameters used for training for a given evaluation dataframe."""
    if model.lower() == "knn":
